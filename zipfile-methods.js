...
import { askForPassword, showLoadingFiles, confirmedFile,userInquirer,clearScreen } from './inquirer-methods.js';
...
async function isConfirmed(latestModifiedFile,addFiles) {
    ...
    if(confirmed) {
        unzipFile(latestModifiedFile.filePath,outputPath,latestModifiedFile.sourceType, null,addFiles)
...
async function needsPasswordUnzip(latestModifiedFile,addFiles) {
    try {
        if (latestModifiedFile.needsPWD) {
            // let {zipPassword} = await askForPassword();
            ...
            let {zipPassword} = await userInquirer(prompts);
                let {fileSize,fileName} = 
                await unzipFile(latestModifiedFile.filePath,outputPath,latestModifiedFile.sourceType, zipPassword,addFiles)
                deleteFromAddList(latestModifiedFile.filePath,addFiles)            
...
async function chooseTargetFile(addFiles) {
    ...
    // let targetFile = await showLoadingFiles(showList)
...
async function unzipFile(filePath,outputPath,sourceType,unzipPassword,addFiles) {
        ...
        writeStream.on('finish', () => {
            console.log(
                chalk.white.bgGreen(`解压成功`) 
                + chalk.white(`    文件名:${decodedPath}  文件大小:${(directory.files[0]['uncompressedSize']/1024).toFixed(1)}KB`)
            )
            resolve({
